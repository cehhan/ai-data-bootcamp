...
- 중복: 321건
- 이상치: revenue 음수(-45,000), quantity(50) 이상치
...
7. 이상치(outlier) 처리: revenue, quantity에서 IQR판단 기준으로 이상치 발생. revenue의 경우 음수 케이스 발생으로 4.항목 처리에 따름. quantity의 경우 이상치 값을 필터링 하여 추출하여 보았을 때 계산 실수 또는 오입력은 없었으며 매장별로 비교해보았을 때에도 균등하게 발생, 유지
